...
shapes_to_load = [
    {
        "stl_path": r'C:\Users\N-ONE\projects\shape_data\sample2_out.stl',
        "alpha": 0.2,
        "color": "lightblue"
    },
    {
        "stl_path": r'C:\Users\N-ONE\projects\shape_data\sample2_in.stl',
        "alpha": 0.8,
        "color": "lightgreen"
    }
]
...
print("\n--- ジオメトリ準備完了 ---")
print("FDTD計算に必要な以下のグリッドが生成されました:")
print(f"- object_id_grid (形状: {object_id_grid.shape}, ユニークID: {np.unique(object_id_grid)})")
print(f"- inside_mask (形状: {inside_mask.shape}, 内部点の数: {np.sum(inside_mask)})")
print(f"- boundary_mask (形状: {boundary_mask.shape}, 境界点の数: {np.sum(boundary_mask)})")
print(f"- impedance_grid (形状: {impedance_grid.shape})")
print(f"- normals_voxcel (形状: {normals_voxcel.shape})")