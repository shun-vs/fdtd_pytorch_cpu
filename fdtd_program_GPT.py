...
microphones = [
    {
        "position": [0.02, 0.0, 0.0],  # マイクの物理座標 [m]
        "output_wav_path": "./mic1_output_test8.wav",
        "process": "raw" # 生の音圧を記録
    },
    {
        "position": [0.02, 0.0, 0.0],
        "output_wav_path": "./mic2_output_test8.wav",
        "process": "integrate" # 時間積分した音圧を記録
    },
]

# --- PML吸収境界の設定 ---
pml_settings = {
    "layers": 20,       # PML層の厚さ（グリッド数）
...
# グリッド、計算時間、出力ファイル
dx = dy = dz = 0.005 # 空間ステップ [m]
tmax = 0.04 # 計算時間 [s]
output_path = r'D:\FDTD_animation\test\fdtd_animation_mictest8.mp4' # 出力ファイルのパス
...
# --- 時間ステップdtの計算方法を選択 ---
synchronize_dt_to_rec_rate = False # True: 録音周波数にdtを同期, False: CFL限界(Courant=1)にdtを最大化
...
# 安定限界となる時間ステップの上限を計算
cfl_limit_dt = 1 / (c0 * np.sqrt(1/dx**2 + 1/dy**2 + 1/dz**2))

if synchronize_dt_to_rec_rate:
    # --- 方法1: 録音周波数に同期 ---
    print("\n時間ステップ(dt)を録音周波数に同期します。")
    rec_dt = 1.0 / rec_sampling_rate
    n_steps_per_rec = max(1, np.ceil(rec_dt / cfl_limit_dt))
    dt = rec_dt / n_steps_per_rec
else:
    # --- 方法2: クーラン数を最大化 ---
    print("\n時間ステップ(dt)をクーラン数=1に最大化します。")
        # 浮動小数点誤差を避け、安定性を確保するため安全率を乗算
    courant_safety_factor = 0.95 # 安全率 (0.9〜0.99程度)
    dt = cfl_limit_dt * courant_safety_factor
    # # 浮動小数点誤差を避けるため、ごくわずかに小さい値を設定
    # dt = np.floor(cfl_limit_dt * 1e13) / 1e13
    n_steps_per_rec = (1.0 / rec_sampling_rate) / dt

tx = int(round(tmax / dt))
fs_fdtd = 1.0 / dt # 最終的なFDTDのサンプリング周波数

print(f"  - FDTD時間ステップ(dt): {dt:.8f} s")
print(f"  - FDTDサンプリング周波数: {fs_fdtd:.2f} Hz")
...
# 録音の間引き間隔は、dtの決定時に計算済み
rec_interval_steps = int(n_steps_per_rec)
...
for i, mic_info in enumerate(microphones):
    ...
    process_mode = mic_info.get("process", "raw") # デフォルトは"raw"
    ...
    if (0 <= ix < nx) and (0 <= iy < ny) and (0 <= iz < nz) and inside_mask[ix, iy, iz]:
        recorders.append({
            "index": (ix, iy, iz),
            "path": path,
            "process": process_mode, # 処理モードを保存
            "history": np.zeros(num_rec_samples),
            "rec_counter": 0
        })
        print(f"- マイク {i+1} ({process_mode}モード) を位置 ({ix}, {iy}, {iz}) に設定しました。")
    else:
...
def update_frame(t):
    ...
    # 順序3: 音圧更新 (ここから下を全て置き換え)
    # --- Step 4.1: 通常領域の音圧更新 ---
    # まず、計算領域全体で、PMLを考慮しない標準的な音圧更新を行う
    div_vx = (vx[1:nx+1] - vx[:nx])
    div_vy = (vy[:, 1:ny+1] - vy[:, :ny])
    div_vz = (vz[:, :, 1:nz+1] - vz[:, :, :nz])
    
    # is_not_in_pmlマスクを使い、通常領域(PML外)のpを更新する
    # (p = p - kappa * dt * div(v) と等価)
    p[is_not_in_pml] -= (kappa * dt) * (div_vx[is_not_in_pml]/dx + div_vy[is_not_in_pml]/dy + div_vz[is_not_in_pml]/dz)

    # --- Step 3.2: PML領域の音圧更新 ---

    # PML領域でのみ、分離場を用いた更新を行う
    if np.any(is_in_pml): # PML領域が存在する場合のみ計算
        p_x[is_in_pml] = C1_x[is_in_pml] * p_x[is_in_pml] + C2_x[is_in_pml] * div_vx[is_in_pml]
        p_y[is_in_pml] = C1_y[is_in_pml] * p_y[is_in_pml] + C2_y[is_in_pml] * div_vy[is_in_pml]
        p_z[is_in_pml] = C1_z[is_in_pml] * p_z[is_in_pml] + C2_z[is_in_pml] * div_vz[is_in_pml]
        
        # pの値を、PML領域でのみ分離場の合計値で上書きする
        p[is_in_pml] = p_x[is_in_pml] + p_y[is_in_pml] + p_z[is_in_pml]
    ...
    for source in source_data_list:
        ...
        if t < len(pin_waveform) and inside_mask[ix, iy, iz]:
            p[ix, iy, iz] += pin_waveform[t] * kappa * dt

                
    # 順序5: マイクの録音
    if t % rec_interval_steps == 0:
        rec_idx = t // rec_interval_steps
...
sampling_rate = rec_sampling_rate # 書き出し周波数は設定値そのもの

for recorder in recorders:
    path = recorder["path"]
    process_mode = recorder["process"]
    pressure_history = recorder["history"]
    
    if np.max(np.abs(pressure_history)) > 0:
        
        # --- 積分処理 ---
        if process_mode == 'integrate':
            print(f"  - {path} の音圧データを時間積分します...")
            # 累積和を使って離散時間積分を実行
            integrated_pressure = np.cumsum(pressure_history) * dt
            # 処理後の波形を дальнейшей обработкиに渡す
            data_to_process = integrated_pressure
        else: # "raw" または指定なし
            data_to_process = pressure_history
            
        # --- 正規化と16bit変換 ---
        # 最大絶対値で割り、[-1, 1]の範囲に正規化
        normalized_data = data_to_process / np.max(np.abs(data_to_process))
        # 16bit整数の範囲にスケーリング
        data_to_write = np.int16(normalized_data * 32767)
