...
# process=Falseで元のデータをまず読み込む
mesh = tri.load_mesh(stl_path, process=False)

### ▼▼▼ 新しい修正箇所: 統合的なメッシュ前処理 ▼▼▼ ###
# trimeshの統合処理機能(process)を呼び出し、メッシュの品質を自動で修正
# これには、法線の向きの統一、重複頂点の削除、穴埋めなどが含まれる
print("メッシュの前処理を実行します (mesh.process)...")
mesh.process(validate=True)
# 念のため、再度水密性を確認し、必要であれば穴を埋める
if not mesh.is_watertight:
    print("警告: process後もメッシュが水密ではありません。穴を塞ぎます。")
    mesh.fill_holes()
### ▲▲▲ 修正完了 ▲▲▲ ###
...
z_slice_index = iz_source # 音源のzスライスを表示
...
def update_frame(t):
    ...
    vx[1:nx][inside_mask[1:nx] & inside_mask[:nx-1]] -= (dt / rho0) * grad_p_x[inside_mask[1:nx] & inside_mask[:nx-1]]
    grad_p_y = (p[:, 1:ny] - p[:, :ny-1]) / dy
    vy[:, 1:ny][inside_mask[:, 1:ny] & inside_mask[:, :ny-1]] -= (dt / rho0) * grad_p_y[inside_mask[:, 1:ny] & inside_mask[:, :ny-1]]
    grad_p_z = (p[:, :, 1:nz] - p[:, :, :nz-1]) / dz
    vz[:, :, 1:nz][inside_mask[:, :, 1:nz] & inside_mask[:, :, :nz-1]] -= (dt / rho0) * grad_p_z[inside_mask[:, :, 1:nz] & inside_mask[:, :, :nz-1]]
    ...
    p[inside_mask] -= kappa * dt * divergence[inside_mask]
